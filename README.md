...
The initial calibration is **18,600,000 weight units per account percentage point**, rounded up from the largest observed value, 18,594,388, across the ten retained completed intervals from September 7, 2026, 02:39 to 17:52 UTC. This chooses the lowest observed percentage-per-weight conversion to reduce over-assignment. The sample is small and may include untracked activity or reporting delays, so this is a provisional estimate, not a measured quota budget or an accuracy guarantee. The setting is saved in both Wrangler configurations and never recalibrates automatically. Changing it intentionally recalculates the estimates; do not adjust it just to make members add up to the account total.

Unattributed usage is the positive difference between the account reading and member estimates. It includes unreported activity **and estimation error**, so it is not proof of unknown-device consumption. If member estimates exceed the account reading, the UI shows that discrepancy and leaves member estimates unchanged. Unknown models receive no invented rate and make the affected member's estimate partial. Older reports without service tiers use the standard-rate weight and are also marked partial.
