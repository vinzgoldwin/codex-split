...
Quota observations are retained in `quota_samples` with their window until that window is pruned. Repeated identical observations are deduplicated; stale observations cannot replace a newer dashboard reading. They are audit evidence, never allocation inputs. When the first quota reading arrives after request uploads, retained protocol 2 batches are queued for reconciliation into their original usage window. Each successful sync reconciles at most one batch through the existing atomic repricing mechanism. Reports whose raw batches have already expired cannot be reconstructed this way.
